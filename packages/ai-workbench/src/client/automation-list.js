...
export function createAutomationList(React, options = {}) {
  ...
  return function AutomationList({ workbench } = {}) {
    ...
    return h("section", { className: "daw-automation-page", "aria-labelledby": "daw-automation-title" }, [
      h("header", { className: "daw-automation-page-header", key: "header" }, [h("div", { key: "intro" }, [h("h1", { id: "daw-automation-title", key: "title" }, "已安排的任务"), h("p", { key: "copy" }, "让 DSH 按计划提醒你，或自动完成重复的 Work 任务")]), h("button", { type: "button", className: "daw-send-button", onClick: () => setEditor({}), key: "create" }, "创建")]),
      ...
      h("section", { className: "daw-automation-suggestions", key: "suggestions" }, [h("h2", null, "建议"), h("p", null, "从常用模板开始安排"), h("button", { type: "button", onClick: () => setEditor({ name: "每日简报", type: "reminder", prompt: "整理今天的待办并提醒我。", schedule: { kind: "daily", time: "08:00" }, timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC", enabled: true }) }, "每日简报 · 工作日 08:00")]),
      editor ? h(AutomationEditor, { item: editor.id ? editor : null, onClose: () => setEditor(null), onSaved: refresh, workbench, key: "editor" }) : null,
      detail ? h(AutomationDetail, { item: detail, onClose: () => setDetail(null), workbench, key: "detail" }) : null,
    ]);
  };
}
