...
export async function createRepository(unit) {
  ...
  return {
    ...
    getAutomationRun: (id) => get("automation_runs", id),
    getAutomationRunByKey: (idempotencyKey) => list("automation_runs").find((record) => record.idempotencyKey === idempotencyKey) || null,
    putAutomationRun: (record) => put("automation_runs", record.id, record),
    deleteAutomationRun: (id) => remove("automation_runs", id),
