...
try {
  ...
  const external = await skill(join(temp, "external", "review"), "external-review");
  await link(external, join(agents.path, "alias"));
  assert.ok((await core.scanEntries(agents.path)).entries.some(e => e.name === "alias"), "默认发现未知外部目录链接");
  assert.equal((await core.skillDetail("agents", "alias")).body, "external-review 正文");
  const candidate = (await core.listProviderCandidates()).find(e => e.name === "external-review");
  assert.equal((await core.getProviderSkill(candidate)).content, "external-review 正文");
  assert.equal((await core.deleteSkill(agents.path, "alias")).code, "error.root.readonly");

  await link(external, join(codex.path, "group", "other-alias"));
  await core.setSkillEnabled(codex.path, "group/other-alias", false);
  assert.equal((await core.readManagerState()).writable, true, "递归路径策略可往返保存");
  assert.equal((await core.listProviderCandidates()).find(e => e.name === "external-review").invocation.modelInvocable, false, "不同名称的分发入口继承停用");
  assert.equal((await core.skillDetail("codex", "group/other-alias")).code, "error.skill.notFound", "详情隐藏已去重入口");
  await core.setSkillEnabled(agents.path, "alias", true);
  assert.equal((await core.listProviderCandidates()).find(e => e.name === "external-review").invocation.modelInvocable, true);
  assert.equal(await core.resolveEntry(agents.path, "../external/review"), null);
  ...
  await skill(join(agents.path, "a", "b", "c", "d", "e", "f", "too-deep"), "too-deep");
  ...
  assert.ok(!scan.entries.some(e => e.name.includes("loop") || e.name.includes(".hidden") || e.declaredName === "too-deep"));
  assert.equal(scan.truncated, true, "超深目录报告扫描截断");
  assert.ok((await core.state()).warnings.some(w => w.code === "warning.scan.truncated"));
  await core.setSkillEnabled(agents.path, "nested/review", false);
  assert.equal((await core.listProviderCandidates()).find(e => e.name === "nested-review").invocation.userInvocable, false);

  await link(join(temp, "missing-target"), join(agents.path, "broken"));
  assert.equal(await core.resolveEntry(agents.path, "broken"), null, "失效目录链接被忽略");
  await mkdir(join(agents.path, "file-link"), { recursive: true });
  try {
    await symlink(join(external, "SKILL.md"), join(agents.path, "file-link", "SKILL.md"), "file");
    assert.equal(await core.resolveEntry(agents.path, "file-link"), null, "不跟随 SKILL.md 文件链接");
  } catch (error) {
    if (!["EPERM", "EACCES", "ENOSYS"].includes(error.code)) throw error;
    console.log("当前系统不允许创建文件软链接；仅此用例跳过，目录 junction 用例已执行");
  }
  ...
  assert.equal((await core.listProviderCandidates()).find(e => e.name === "root-skill").invocation.userInvocable, false);

  const project = join(temp, "project");
  await mkdir(join(project, ".git"), { recursive: true });
  await link(join(temp, "external"), join(project, ".agents", "skills"));
  const projectRoot = (await core.projectRoots([project])).find(r => r.kind === "project-agents");
  assert.ok(projectRoot, "项目只读技能根本身允许链接");
  assert.equal((await core.skillDetail(projectRoot.key, "review", { projectCwds: [project] })).body, "external-review 正文");
  const projectCandidate = (await core.listProviderCandidates({ cwd: project })).find(e => e.name === "external-review");
  assert.equal(projectCandidate.source, "project-agents");
  assert.equal((await core.getProviderSkill(projectCandidate, { cwd: project })).content, "external-review 正文");
  await core.setSkillEnabled(projectRoot, "review", false);
  assert.equal((await core.listProviderCandidates({ cwd: project })).find(e => e.name === "external-review").invocation.userInvocable, false);
  assert.equal((await core.deleteSkill(projectRoot, "review")).code, "error.root.readonly");
  await skill(join(projectRoot.path, "group", "project-only"), "project-only");
  const soleProject = (await core.listProviderCandidates({ cwd: project })).find(e => e.name === "project-only");
  ...
  assert.equal(await core.getProviderSkill(candidate), undefined, "旧候选拒绝改指的链接");
  assert.match(await readFile(join(external, "SKILL.md"), "utf8"), /external-review 正文$/);

  // 真实宽目录验证预算，避免错误地把达到上限的扫描宣称为完整结果。
  const wide = join(temp, "wide");
  await mkdir(wide);
  for (let start = 0; start < 2005; start += 100) {
    await Promise.all(Array.from({ length: Math.min(100, 2005 - start) }, (_, i) => mkdir(join(wide, `dir-${start + i}`))));
  }
