...
- 发布正方 AI 工作台 V1.0，提供独立的 Work 与 Chat 模式。
- 新建和历史工作台会话统一进入 DSH 原生对话界面，同时保留工作台固定左侧栏。
- Work 与 Chat 输入框接入 DSH 原生命令菜单、技能候选、工作区绑定和 prepared 会话恢复。
- 新增原生会话结果封装、工作区就绪、陈旧 prepared 会话、模式隔离等回归测试，并完成工作台包门禁验证。
