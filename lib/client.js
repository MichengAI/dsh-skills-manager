...
`;
    ...
    function SkillManagerSection() {
      ...
      function selectFile(file) {
        ...
        if (!file.path) { selectDirectory(); return; }
        ...
        return h("div", { key: skill.name, className: "dssm-row" },
          h("div", { className: "dssm-row-main" },
            h("div", { className: "dssm-row-id" },
              ...
              h("span", { className: "dssm-tag " + (skill.modelInvocable ? "dssm-tag-on" : "dssm-tag-off") }, skill.modelInvocable ? "已启用" : "已停用")),
            h("div", { className: "dssm-note", title: skill.description || "" }, skill.description || "未提供简介")),
          h("button", { className: "dssm-btn dssm-btn-secondary", disabled: !!busy, onClick: function () { action(skill.modelInvocable ? "/disable" : "/enable", { name: skill.name, root: root.key }); } }, skill.modelInvocable ? "停用" : "启用"),
      ...
      if (snapshot.data) {
        ...
        nodes.push(h("div", { key: "toolbar", className: "dssm-toolbar" },
          h("div", null, h("h2", { className: "dssm-title" }, "技能"), h("p", { className: "dssm-desc" }, "DSH 技能可上传和删除；公共 Agent 技能仅支持启用或停用。")),
      ...
      if (importResult) {
        nodes.push(h("div", { key: "import-result", className: importResult.error ? "dssm-error" : "dssm-note", role: "status" }, importResult.error ? "上传失败：" + importResult.error : "上传完成：" + (importResult.imported || []).map(function (item) { return item.name; }).join("、")));
      }
      if (uploadOpen) {
