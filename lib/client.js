...
`;
    ...
    function translateError(t, payload) {
      if (payload && typeof payload === "object") {
        ...
        if (typeof code === "string" && code !== "") {
          ...
          // locale miss 时 t 返回 key 本身（或 undefined），此时回退原文。
          if (translated !== undefined && translated !== code) return translated;
        }
    ...
    // 词典命中返回翻译，miss（t 返回 key 本身或 undefined）回退 fallback。
    function translateOrFallback(t, key, fallback) {
      var value = t(key);
      return value !== undefined && value !== key ? value : fallback;
    }
    ...
    // 导出词典供零依赖对齐测试读取（宿主只消费 apply/inject，不影响运行时）。
