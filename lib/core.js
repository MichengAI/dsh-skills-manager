...
function entryPath(root, name) {
  if (typeof name !== "string" || name === "" || name === "." || name === ".." || /[\\/\0]/.test(name) || /[. ]$/.test(name) || basename(name) !== name) return null;
...
async function assertNoSymbolicLinks(source) {
  ...
  if (st.isSymbolicLink()) throw new Error(`不支持包含符号链接的 skill 来源: ${source}`);
  if (!st.isDirectory()) return;
  const items = await fs.readdir(source, { withFileTypes: true });
  for (const item of items) await assertNoSymbolicLinks(join(source, item.name));
}
...
export async function importSkill(source, log, options = {}) {
  ...
  if (pathsOverlap(source, targetRoot)) return { ok: false, error: "导入来源不能与 DSH 技能目录相同、包含或位于其中" };
