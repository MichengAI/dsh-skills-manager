...
//   - 项目根目录：<project>/.dsh/skills、<project>/.agents/skills（只读展示）
...
export async function projectRoots(projectCwds = [], diagnostics) {
  ...
  for (const project of [...projects.values()].sort((a, b) => a.root.localeCompare(b.root))) {
    ...
    roots.push(
      { ...common, key: `project-dsh:${id}`, kind: "project-dsh", localeKey: "projectDsh", path: join(project.root, ".dsh", "skills"), label: "Project DSH", rank: 100 },
...
export async function state(options = {}) {
  ...
  for (const root of [...scoped, ...user]) {
    ...
    if (root.scope === "project" && !exists) continue;
    const skills = [];
