...
//   - 项目根目录：<project>/.dsh/skills（可启停、创建、回收）、<project>/.agents/skills（只读展示）
...
export async function projectRoots(projectCwds = [], diagnostics) {
  ...
  for (const cwd of Array.isArray(projectCwds) ? projectCwds : []) {
    ...
    if (!found) {
      if (Array.isArray(diagnostics) && typeof cwd === "string" && isAbsolute(cwd)) {
  ...
  for (const project of [...projects.values()].sort((a, b) => a.root.localeCompare(b.root))) {
    ...
    roots.push(
      ...
      { ...common, key: `project-agents:${id}`, kind: "project-agents", localeKey: "projectAgents", path: join(project.root, ".agents", "skills"), label: "Project Agent", rank: 200 },
    );
  }
  return roots;
}
...
export async function setSkillEnabled(root, name, enabled, log) {
  let definition = rootDefinition(root);
  if (!definition || !definition.toggleable) return readonlyError("toggle");
  if (definition.key !== "dsh" && definition.kind !== "project-dsh") return setReadOnlySkillEnabled(definition, name, enabled, log);
  definition = await checkedWritableRootDefinition(definition);
  ...
  return { root: definition.key, name, enabled };
}
...
function markWinners(items, options = {}) {
  ...
  for (const [canonicalName, winner] of winners) {
    ...
    if (options.setLoaded === false) winner.view.discovered = true;
    else winner.view.loaded = winner.managerEnabled && winner.entry.modelInvocable && winner.entry.userInvocable;
...
export async function state(options = {}) {
  ...
  for (const root of [...scoped, ...user]) {
    ...
    for (const e of entries) {
      const managerEnabled = root.scope === "project" || root.key === "dsh" ? true : managerSkillEnabled(policyResult.state, root.key, e.name);
      skills.push({
        name: e.name,
    ...
    result.roots.push({
      ...
      exists,
      enabled: root.scope === "project" || root.key === "dsh" ? true : policyResult.state.sources[root.key] !== false,
      skills,
    });
  }
  ...
  for (const projectItems of projectGroups.values()) {
    ...
    markWinners([...projectItems, ...userCopies], { setLoaded: false });
  }
