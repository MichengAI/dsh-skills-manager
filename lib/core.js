...
/** 真实落地路径必须仍在技能根内，避免符号链接写出根外。 */
async function isInsideRoot(root, path) {
  return isSameOrDescendant(await resolvedPath(root), await resolvedPath(path));
}
...
export function entryPath(root, name) {
  if (typeof name !== "string" || name === "" || name === "." || name === ".." || name.startsWith(".") || /[\\/:*?"<>|\0]/.test(name) || /[. ]$/.test(name) || WINDOWS_DEVICE_NAME_RE.test(name) || basename(name) !== name) return null;
...
export async function resolveEntry(root, name) {
  ...
  if (bundleStat && bundleStat.isDirectory() && !bundleStat.isSymbolicLink()) {
    ...
    if (docStat && docStat.isFile() && !docStat.isSymbolicLink() && await isInsideRoot(rootPath, bundleDoc)) {
  ...
  if (flatStat && flatStat.isFile() && !flatStat.isSymbolicLink() && await isInsideRoot(rootPath, flatDoc)) {
  ...
  for (const it of items) {
    try {
      ...
      if (it.isDirectory() && entryPath(root, it.name) !== null) {
        ...
        if (!st || !st.isFile() || st.isSymbolicLink() || !(await isInsideRoot(root, docPath))) continue;
        ...
        if (!(await isInsideRoot(root, docPath))) continue;
        const doc = parseSkillDoc(await fs.readFile(docPath, "utf8"));
