...
export async function browseDirectories(inputPath) {
  ...
  try {
    ...
    for (const item of items) {
      ...
      entries.push({ name: item.name, path: join(canonical, item.name), hidden: item.name.startsWith(".") });
    }
  } catch (error) {
...
export async function readManagerState() {
  try {
    ...
    return { state: normalizeManagerState(JSON.parse(raw)), warning: null };
  } catch (error) {
    if (error && error.code === "ENOENT") return { state: defaultManagerState(), warning: null };
    return {
      state: defaultManagerState(),
      warning: { code: "warning.state.invalid", params: { path: managerStatePath() }, error: `技能管理器状态文件不可读，已使用默认状态: ${managerStatePath()}` },
    };
  }
}
...
export async function deleteSkill(root, name, log) {
  ...
  try {
    for (const target of targets) {
      ...
      await renameWithRetry(target.path, destination);
    ...
    await renameWithRetry(stage, finalPath);
    if (log) log("trash", `移到回收站 ${name} -> ${finalPath}`);
    return { id, name, deletedAt: metadata.deletedAt };
  } catch (error) {
    for (const item of moved.reverse()) await renameWithRetry(item.destination, item.path).catch(() => undefined);
