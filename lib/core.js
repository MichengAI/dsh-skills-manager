...
async function isInsideRoot(root, path) {
  return isInsideResolvedRoot(await resolvedPath(root), path);
}
...
export async function importSkill(source, log, options = {}) {
  ...
  // 全空候选已在上方以 failureResult 返回；dry-run 仅需输出预检结果。
  if (dryRun) return { kind: analysis.kind, pending, conflicts, failed };
