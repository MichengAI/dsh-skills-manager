...
function migrateManagerStateDocument(value) {
  if (!value || typeof value !== "object" || Array.isArray(value) || value.version !== 1)
    return value;
  if (value.sources && typeof value.sources === "object" && !Array.isArray(value.sources) && value.sources.ccswitch === void 0)
    value.sources.ccswitch = true;
  if (value.disabledSkills && typeof value.disabledSkills === "object" && !Array.isArray(value.disabledSkills) && value.disabledSkills.ccswitch === void 0)
    value.disabledSkills.ccswitch = [];
  if (value.enabledSkills && typeof value.enabledSkills === "object" && !Array.isArray(value.enabledSkills) && value.enabledSkills.ccswitch === void 0)
    value.enabledSkills.ccswitch = [];
  if (value.sources && typeof value.sources === "object" && !Array.isArray(value.sources) && value.sources.cursor === void 0)
    value.sources.cursor = true;
  if (value.disabledSkills && typeof value.disabledSkills === "object" && !Array.isArray(value.disabledSkills) && value.disabledSkills.cursor === void 0)
    value.disabledSkills.cursor = [];
  if (value.enabledSkills && typeof value.enabledSkills === "object" && !Array.isArray(value.enabledSkills) && value.enabledSkills.cursor === void 0)
...
async function checkedPolicyRootDefinition(root) {
  ...
  const validProjectRoot = (definition.kind === "project-dsh" || definition.kind === "project-agents") && PROJECT_ROOT_KEY_RE.test(definition.key) && typeof definition.projectRoot === "string" && isAbsolute(definition.projectRoot) && definition.key === `${definition.kind}:${projectIdentity(definition.projectRoot)}` && resolve(definition.path) === resolve(
    join(
      definition.projectRoot,
      definition.kind === "project-dsh" ? ".dsh" : ".agents",
      "skills"
...
async function listProviderCandidates(options = {}) {
  ...
  const items = user.flatMap(
    (root) => userScans.get(root.key).entries.map((entry) => ({ root, entry }))
  );
...
async function state(options = {}) {
  ...
  result.summary = {
    ...
    issues: all.reduce(
      (count, item) => count + item.entry.diagnostics.length + (item.view.shadowedBy ? 1 : 0),
      0
    )
  };
  return result;
