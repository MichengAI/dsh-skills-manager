...
// ── 启用 / 停用（改 frontmatter 的 disable-model-invocation，非破坏）─────────

/** enabled=true 移除 disable-model-invocation；false 置为 true。 */
...
export async function importSkill(source, log, options = {}) {
  ...
  for (const c of candidates) {
    ...
    if (exists) {
      conflicts.push({ name: c.kebab, source: c.source, isDir: c.isDir });
      continue;
    }
