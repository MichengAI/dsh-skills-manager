 ...
 */
async function resolveTrustedLinkedBundle(root, bundlePath) {
  ...
  let sourceRootReal;
  try {
    sourceRootReal = await fs.realpath(sourceRoot.path);
  } catch {
    return null;
  }
  let trusted = false;
  for (const targetRoot of userRoots().filter((candidate) => !candidate.mutable && candidate.scope !== "project")) {
    const targetRootStat = await lstatOrNull(targetRoot.path);
    if (!targetRootStat || !targetRootStat.isDirectory() || targetRootStat.isSymbolicLink()) continue;
    const targetRootReal = await fs.realpath(targetRoot.path);
    if (pathIdentity(targetRootReal) === pathIdentity(sourceRootReal)) continue;
    const acceptedTarget = entryPath(targetRootReal, basename(realEntryPath));
  ...
  for (const it of items) {
    try {
      if (it.isSymbolicLink()) {
        ...
        const linked = await resolveTrustedLinkedBundle(root, bundlePath);
        if (!linked) continue;
...
/** 同一真实技能可能被 CC Switch 分发到多个 Agent 根；按来源 rank 只保留一份。 */
async function scanDeduplicatedUserRoots(roots = userRoots()) {
  const scans = new Map();
  for (const root of roots) scans.set(root.key, await scanEntries(root.path));
  const claimed = new Set();
  for (const root of [...roots].sort((left, right) => left.rank - right.rank)) {
    const scan = scans.get(root.key);
...
/** v0.1.32 及更早状态尚无 CC Switch 键；只补充新增字段，其余损坏仍然失败关闭。 */
...
export async function skillDetail(key, name, options = {}) {
  ...
  const raw = await fs.readFile(entry.realDocPath || entry.docPath, "utf8");
...
export async function getProviderSkill(candidate, options = {}) {
  ...
  if (locator.realEntryPath && pathIdentity(entry.realEntryPath) !== pathIdentity(locator.realEntryPath)) return undefined;
  if (locator.realDocPath && pathIdentity(entry.realDocPath) !== pathIdentity(locator.realDocPath)) return undefined;
