...
//   - 项目根目录：<project>/.dsh/skills（可创建、回收）、<project>/.agents/skills（只读展示）
...
export async function projectRoots(projectCwds = [], diagnostics) {
  ...
  for (const project of [...projects.values()].sort((a, b) => a.root.localeCompare(b.root))) {
    ...
    roots.push(
      { ...common, key: `project-dsh:${id}`, kind: "project-dsh", localeKey: "projectDsh", path: join(project.root, ".dsh", "skills"), label: "Project DSH", rank: 100, mutable: true },
