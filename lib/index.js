...
// - $DSH_HOME\skills 可上传、删除、启停；$DSH_AGENTS_HOME\skills 仅允许启停
      ...
      try {
        ...
        switch (path) {
          case "/api/dsh-skills-manager/enable": {
            const rootPath = rootByKey[String(body.root || "dsh")];
            run(res, rootPath ? setSkillEnabled(rootPath, String(body.name || ""), true) : Promise.resolve({ ok: false, error: "无效的技能目录" }));
            return;
          }
          case "/api/dsh-skills-manager/disable": {
            const rootPath = rootByKey[String(body.root || "dsh")];
            run(res, rootPath ? setSkillEnabled(rootPath, String(body.name || ""), false) : Promise.resolve({ ok: false, error: "无效的技能目录" }));
            return;
          }
          case "/api/dsh-skills-manager/delete": {
            run(res, deleteSkill(rootByKey.dsh, String(body.name || ""), log));
            return;
          }
