      ...
      try {
        ...
        switch (path) {
          case "/api/dsh-skills-manager/enable": {
            run(res, String(body.root || "dsh") === "dsh" ? setSkillEnabled(rootByKey.dsh, String(body.name || ""), true) : Promise.resolve({ ok: false, error: "公共 Agent 技能目录不允许启用或停用", code: "error.root.readonly", params: { action: "enable" } }), invalidateSkills);
            return;
          }
          case "/api/dsh-skills-manager/disable": {
            run(res, String(body.root || "dsh") === "dsh" ? setSkillEnabled(rootByKey.dsh, String(body.name || ""), false) : Promise.resolve({ ok: false, error: "公共 Agent 技能目录不允许启用或停用", code: "error.root.readonly", params: { action: "disable" } }), invalidateSkills);
            return;
          }
